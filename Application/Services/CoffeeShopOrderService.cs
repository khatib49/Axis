...
namespace Application.Services
{
    ...
    {
        private readonly IBaseRepository<CoffeeShopOrder> _repo;
        private readonly IUnitOfWork _uow;
        private readonly DomainMapper _mapper;

        public CoffeeShopOrderService(IBaseRepository<CoffeeShopOrder> repo, IUnitOfWork uow, DomainMapper mapper)
        {
            _repo = repo; _uow = uow; _mapper = mapper;
        }

        public async Task<CoffeeShopOrderDto?> GetAsync(Guid id, CancellationToken ct = default)
        {
            var e = await _repo.GetByIdAsync(id, asNoTracking: true, ct);
            return e is null ? null : _mapper.ToDto(e);
        }

        public async Task<PaginatedResponse<CoffeeShopOrderDto>> ListAsync(BasePaginationRequestDto pagination, CancellationToken ct = default)
        {
        ...
        public async Task<bool> UpdateAsync(Guid id, CoffeeShopOrderUpdateDto dto, CancellationToken ct = default)
        {
